const redirects: Record<number, Record<string, boolean>> = {};
const onBeforeRedirect = (d: any) => {
...
const onHeadersReceived = (d: any) => {
  ...
  const o = responseHeaders.find(
    ({ name }: any) => name.toLowerCase() === "access-control-allow-origin"
  );
